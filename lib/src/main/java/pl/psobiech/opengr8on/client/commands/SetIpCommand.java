...
public class SetIpCommand {
    ...
    public static boolean requestMatches(byte[] buffer) {
        if (buffer.length < Request.COMMAND.length() + 1 + Command.MIN_SERIAL_NUMBER_SIZE + 1 + Command.MIN_IP_SIZE + 1 + Command.MIN_IP_SIZE) {
            return false;
        }
    ...
    public static boolean responseMatches(byte[] buffer) {
        if (buffer.length < Response.COMMAND.length() + 1 + Command.MIN_SERIAL_NUMBER_SIZE + 1 + Command.MIN_IP_SIZE) {
            return false;
        }
